...
class ReduceSpaceInfiltrationByPercentage < OpenStudio::Ruleset::ModelUserScript
  ...
  def arguments(model)
    ...
    puts "in args building.handle.to_s = #{building.handle.to_s}"
  ...
  def run(model, runner, user_arguments)
    ...
    building = model.getBuilding
    puts "in measure building.handle.to_s = #{building.handle.to_s}"
    
    str = runner.getStringArgumentValue("space_type",user_arguments)
    puts "looking for #{str}"
