...
class AirflowTest < MiniTest::Test
  ...
  def test_non_ducted_hvac_equipment
    ...
    assert_equal("Success", result_value(result))
    assert_includes(result_warnings(result)[0], "No ducted HVAC equipment was found but ducts were specified. Overriding duct specification.")
  end  
  ...
  def _test_measure(osm_file_or_model, args_hash)
    ...
    measure = ProcessAirflowOriginalModel.new
