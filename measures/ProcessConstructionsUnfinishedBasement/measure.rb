...
class ProcessConstructionsUnfinishedBasement < OpenStudio::Ruleset::ModelUserScript
  ...
  def arguments(model)
    ...
    selected_living = OpenStudio::Ruleset::OSArgument::makeChoiceArgument("selectedufbsmt", spacetype_handles, spacetype_display_names, true)
  ...
  revunfinbinsfinfloor.setName("RevUnfinBInsFinFloor")
	if carpet.CarpetFloorFraction > 0
    revunfinbinsfinfloor.insertLayer(layercount,cbl)
		layercount += 1
	end
    revunfinbinsfinfloor.insertLayer(layercount,fm)
	  layercount += 1
	if uciRvalue > 0
    revunfinbinsfinfloor.insertLayer(layercount,uci)
		layercount += 1
	end
  revunfinbinsfinfloor.insertLayer(layercount,ply3_4)
