  ...
  def run(model, runner, user_arguments)
    ...
    (0...num_units).to_a.each do |unit_num|
      ...
      control_slave_zones_hash.each do |control_zone, slave_zones|
        ...
        runner.registerInfo("Added baseboard convective electric '#{htg_coil.name}' to thermal zone '#{control_zone.name}' of unit #{unit_num + 1}")
        ...
        slave_zones.each do |slave_zone|
          ...
          runner.registerInfo("Added baseboard convective electric '#{htg_coil.name}' to thermal zone '#{slave_zone.name}' of unit #{unit_num + 1}")
