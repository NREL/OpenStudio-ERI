...
class ResidentialHotWaterHeaterTankGasTest < MiniTest::Test
  ...
  def test_new_construction_standard
    args_hash = {}
    _test_measure(osm_geo_beds_loc, args_hash, 0, 1, 40, Constants.FinishedBasementZone, 11.72, 0.773, 7.88, 125, 0, 0, 1, 0)
  end
  ...
  def test_new_construction_premium
    ...
    args_hash["water_heater_capacity"] = "34"
    args_hash["oncyc_power"] = 165
    args_hash["offcyc_power"] = 1
    _test_measure(osm_geo_beds_loc, args_hash, 0, 1, 40, Constants.FinishedBasementZone, 9.97, 0.789, 4.503, 125, 165, 1, 1, 0)
  end
  ...
  def test_new_construction_standard_living
    ...
    _test_measure(osm_geo_beds_loc, args_hash, 0, 1, 40, Constants.LivingZone, 11.72, 0.773, 7.88, 125, 0, 0, 1, 0)
  end

  def test_new_construction_standard_setpoint_130
    args_hash = {}
    args_hash["dhw_setpoint_temperature"] = 130
    _test_measure(osm_geo_beds_loc, args_hash, 0, 1, 40, Constants.FinishedBasementZone, 11.72, 0.773, 7.88, 130, 0, 0, 1, 0)
  end

  def test_new_construction_standard_volume_30
    args_hash = {}
    args_hash["storage_tank_volume"] = "30"
    _test_measure(osm_geo_beds_loc, args_hash, 0, 1, 30, Constants.FinishedBasementZone, 11.72, 0.773, 7.88, 125, 0, 0, 1, 0)
  end
  ...
  def test_retrofit_replace
    args_hash = {}
    model = _test_measure(osm_geo_beds_loc, args_hash, 0, 1, 40, Constants.FinishedBasementZone, 11.72, 0.773, 7.88, 125, 0, 0, 1, 0)
    args_hash = {}
    args_hash["rated_energy_factor"] = "0.67"
    args_hash["water_heater_recovery_efficiency"] = 0.78
    args_hash["water_heater_capacity"] = "34"
    args_hash["oncyc_power"] = 165
    args_hash["offcyc_power"] = 1
    _test_measure(model, args_hash, 1, 1, 40, Constants.FinishedBasementZone, 9.97, 0.789, 4.503, 125, 165, 1, 1, 0)
  end
  
  def test_retrofit_replace_tank_electric
    args_hash = {}
    _test_measure(osm_geo_beds_loc_tank_electric, args_hash, 1, 1, 40, Constants.FinishedBasementZone, 11.72, 0.773, 7.88, 125, 0, 0, 1, 0)
  end

  def test_retrofit_replace_tank_oil
    args_hash = {}
    _test_measure(osm_geo_beds_loc_tank_oil, args_hash, 1, 1, 40, Constants.FinishedBasementZone, 11.72, 0.773, 7.88, 125, 0, 0, 1, 0)
  end

  def test_retrofit_replace_tank_propane
    args_hash = {}
    _test_measure(osm_geo_beds_loc_tank_propane, args_hash, 1, 1, 40, Constants.FinishedBasementZone, 11.72, 0.773, 7.88, 125, 0, 0, 1, 0)
  end

  def test_retrofit_replace_tankless_electric
    args_hash = {}
    _test_measure(osm_geo_beds_loc_tankless_electric, args_hash, 1, 1, 40, Constants.FinishedBasementZone, 11.72, 0.773, 7.88, 125, 0, 0, 1, 0)
  end

  def test_retrofit_replace_tankless_gas
    args_hash = {}
    _test_measure(osm_geo_beds_loc_tankless_gas, args_hash, 1, 1, 40, Constants.FinishedBasementZone, 11.72, 0.773, 7.88, 125, 0, 0, 1, 0)
  end

  def test_retrofit_replace_tankless_propane
    args_hash = {}
    _test_measure(osm_geo_beds_loc_tankless_propane, args_hash, 1, 1, 40, Constants.FinishedBasementZone, 11.72, 0.773, 7.88, 125, 0, 0, 1, 0)
  end

  def test_multifamily_new_construction
    num_units = 3
    args_hash = {}
    _test_measure(osm_geo_multifamily_3_units_beds_loc, args_hash, 0, num_units, 120, nil, 35.172, 2.319, 23.64, 375, 0, 0, num_units, 0)
  end
  
  def test_multifamily_new_construction_living_zone
    args_hash = {}
    args_hash["water_heater_location"] = "living zone 1"
    _test_measure(osm_geo_multifamily_3_units_beds_loc, args_hash, 0, 1, 40, "living zone 1", 11.72, 0.773, 7.88, 125, 0, 0, 1, 0)
  end
  
  def test_multifamily_new_construction_mult_draw_profiles
    num_units = 12
    args_hash = {}
    _test_measure(osm_geo_multifamily_12_units_beds_loc, args_hash, 0, num_units, 480, nil, 140.7, 9.276, 94.56, 1500, 0, 0, num_units, 0)
  end

  def test_multifamily_retrofit_replace
    num_units = 3
    args_hash = {}
    model = _test_measure(osm_geo_multifamily_3_units_beds_loc, args_hash, 0, num_units, 120, nil, 35.172, 2.319, 23.64, 375, 0, 0, num_units, 0)
    args_hash = {}
    args_hash["rated_energy_factor"] = "0.67"
    args_hash["water_heater_recovery_efficiency"] = 0.78
    args_hash["water_heater_capacity"] = "34"
    args_hash["oncyc_power"] = 165
    args_hash["offcyc_power"] = 1
    _test_measure(model, args_hash, num_units, num_units, 120, nil, 29.896, 2.367, 13.50, 375, 495, 3, num_units, 0)
  end
  ...
  def test_argument_error_re_lt_0
    ...
    result = _test_error(osm_geo_beds_loc, args_hash)
    assert_equal(result.errors[0].logMessage, "Recovery efficiency must be at least 0 and at most 1.")
  end
  ...
  def _test_error(osm_file, args_hash)
    ...
    measure = ResidentialHotWaterHeaterTankGas.new
  ...
  def _test_measure(osm_file_or_model, args_hash, expected_num_del_objects, expected_num_new_objects, expected_tank_vol, expected_location, expected_input_cap, expected_thermal_eff, expected_ua, expected_setpoint, expected_oncycle_power, expected_offcycle_power, num_infos=0, num_warnings=0)
    ...
    assert_in_epsilon(tot_vol, Waterheater.calc_actual_tankvol(expected_tank_vol, Constants.FuelTypeGas, Constants.WaterHeaterTypeTank), 0.01)
