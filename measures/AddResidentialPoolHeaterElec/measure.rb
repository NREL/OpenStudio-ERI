...
class ResidentialPoolHeater < OpenStudio::Ruleset::ModelUserScript
  ...
  def name
    return "Add/Replace Residential Electric Pool Heater"
  end
  
  def description
    return "Adds (or replaces) a residential electric pool heater with the specified efficiency and schedule. The pool is assumed to be outdoors."
  end
  
  def modeler_description
    return "Since there is no Electric Pool Heater object in OpenStudio/EnergyPlus, we look for an ElectricEquipment object with the name that denotes it is a residential electric pool heater. If one is found, it is replaced with the specified properties. Otherwise, a new such object is added to the model."
  end
  ...
  def run(model, runner, user_arguments)
    ...
    space_equipments_g.each do |space_equipment_g| #check for an existing gas range
        if space_equipment_g.gasEquipmentDefinition.name.get.to_s == obj_name_g
            runner.registerInfo("There is already a gas pool heater. The existing gas pool heater will be replaced with the specified electric pool heater.")
    ...
    space_equipments.each do |space_equipment|
        if space_equipment.electricEquipmentDefinition.name.get.to_s == obj_name_e
            has_elec_ph = 1
            runner.registerInfo("There is already an electric pool heater. The existing electric pool heater will be replaced with the specified electric pool heater.")
    ...
    if replace_elec_ph == 1
        runner.registerFinalCondition("The existing electric pool heater has been replaced by one with #{ph_ann.round} kWhs annual energy consumption.")
    elsif replace_g_ph == 1
        runner.registerFinalCondition("The existing gas pool heater has been replaced by an electric pool heater with #{ph_ann.round} kWhs annual energy consumption.")
    else
        runner.registerFinalCondition("An electric pool heater has been added with #{ph_ann.round} kWhs annual energy consumption.")
    end
