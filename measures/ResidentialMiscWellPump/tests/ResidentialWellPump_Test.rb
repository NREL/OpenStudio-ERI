...
class ResidentialWellPumpTest < MiniTest::Test
  ...
  def test_new_construction_electric
    ...
    _test_measure(osm_geo_beds, args_hash, 0, 1, 404.2)
  end
  ...
  def test_new_construction_mult_0_127
    ...
    _test_measure(osm_geo_beds, args_hash, 0, 1, 51.3)
  end
  ...
  def test_retrofit_replace
    args_hash = {}
    args_hash["base_energy"] = 400.0
    model = _test_measure(osm_geo_beds, args_hash, 0, 1, 404.2)
    args_hash = {}
    args_hash["base_energy"] = 200.0
    _test_measure(model, args_hash, 1, 1, 202.1, 1)
  end
  ...
  def test_multifamily_new_construction
    ...
    args_hash = {}
    args_hash["base_energy"] = 400.0
    _test_measure(osm_geo_multifamily_3_units_beds, args_hash, 0, num_units, 1069.8, num_units)
  end
  ...
  def test_multifamily_retrofit_replace
    ...
    args_hash = {}
    args_hash["base_energy"] = 400.0
    model = _test_measure(osm_geo_multifamily_3_units_beds, args_hash, 0, num_units, 1069.8, num_units)
    args_hash = {}
    args_hash["base_energy"] = 200.0
    _test_measure(model, args_hash, num_units, num_units, 534.9, 2*num_units)
  end
  ...
  def _test_measure(osm_file_or_model, args_hash, expected_num_del_objects, expected_num_new_objects, expected_annual_kwh, num_infos=0, num_warnings=0)
    ...
    new_objects.each do |new_object|
        ...
        assert(new_object.name.to_s.start_with?(Constants.ObjectNameWellPump))
