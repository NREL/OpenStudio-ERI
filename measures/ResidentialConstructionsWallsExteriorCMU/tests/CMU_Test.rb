...
class ProcessConstructionsWallsExteriorCMUTest < MiniTest::Test
  ...
  def _test_measure(osm_file_or_model, args_hash, expected_num_del_objects, expected_num_new_objects, expected_values)
    ...
    assert_in_epsilon(expected_values["LayerDensity"], actual_values["LayerDensity"], 0.03)
    assert_in_epsilon(expected_values["LayerSpecificHeat"], actual_values["LayerSpecificHeat"], 0.03)
    assert_in_epsilon(expected_values["LayerIndex"], actual_values["LayerIndex"], 0.03)
