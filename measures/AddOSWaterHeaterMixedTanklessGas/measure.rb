...
class AddOSWaterHeaterMixedTanklessGas < OpenStudio::Ruleset::ModelUserScript
  ...
  def name
    return "AddOSWaterHeaterMixedTanklessGas"
  end
  ...
  def arguments(model)
    ...
    rated_energy_factor.setDisplayName("Rated Energy Factor of Gas Tankless Water Heater. This field is ignored for NCTH and B10 protocols.")
    ...
    water_heater_location.setDisplayName("Thermal Zone where the Gas Tankless Water Heater is located")
    ...
    #make an argument for fuel_type
    fuels = ["Natural Gas", "Propane Gas"]
    fuel_types = osargument::makeChoiceArgument("fuel_type", fuels, true)
    fuel_types.setDisplayName("Type of Fuel Used for Heating")
    args << fuel_types
  ...
  def configure_tank_volume(new_heater)
    info_msg = "Water heater of type instantaneous has tank volume "
  ...
  def configure_heater_capacity(new_heater)
    ...
    capacity = new_heater.getHeaterMaximumCapacity.get
    capacity_ip = OS::convert(capacity, KBtuhr).get
    @runner.registerInfo info_prefix + capacity_ip.to_s
      
  end

  def configure_fuel(new_heater)
    fuel_type = @args[:fuel_type]
    fuel = fuel_type.delete(' ')
    new_heater.setHeaterFuelType(fuel)
  end
  ...
  def list_water_heaters
    ...
    for heater in existing_heaters do
      ...
      if heater.isHeaterMaximumCapacityAutosized
        capacity = "an autosized capacity"
      else
        capacity_si = heater.getHeaterMaximumCapacity.get
        capacity_ip = OS::convert(capacity_si, KBtuhr).get
        capacity = "a capacity of #{capacity_ip}"
      end
      
      if heater.isTankVolumeAutosized
        volume = "an autosized tank volume."
      else
        volume_si = heater.getTankVolume.get
        volume_ip = OS::convert(volume_si, Gallon).get
        # volume_si = heater.tankVolume.get
        # volume_ip = OS::convert(volume_si, "m^3", 'gal').get
        # volume = qty(volume_ip, Gallon)
        if volume_ip.value <= 1
          volume = "is tankless."
        else
          volume = "a #{volume_ip} tank."
        end
      end
      
      water_heaters << "Water heater '#{heatername}' on plant loop '#{loopname}', with #{capacity}" +
        " and #{volume}"
    end
  ...
  def validate_rated_energy_factor
    ...
    if (rated_energy_factor < 0.68)
      @runner.registerWarning("AHRI Certified Energy Factors for Commercially available Gas Tankless Water Heaters should be 0.68 or greater.")
    end
    if (rated_energy_factor > 0.99)
      @runner.registerWarning("AHRI Certified Energy Factors for Commercially available Gas Tankless Water Heaters should be less than 0.99.")
    end    
  end
  ...
  def validate_water_heater_capacity
    water_heater_capacity = @args[:water_heater_capacity]
    if water_heater_capacity.value < 0 
      @runner.registerError("Gas Tankless Water Heater Nominal Capacity must be greater than 0 kBtu/hr.")
    end
    if water_heater_capacity.value < 74.9
      @runner.registerWarning("Commercially Available Gas Tankless Water Heaters should have a minimum Nominal Capacity of 74.9 kBtu/hr.")
    end
    if water_heater_capacity.value > 199.9
      @runner.registerWarning("Commercially Available Gas Tankless Water Heaters should have a maximum Nominal Capacity of 199.9 kBtu/hr.")
    end
  end
