...
class ProcessConstructionsExteriorInsulatedWallsSIP < OpenStudio::Ruleset::ModelUserScript
  ...
  # human readable name
  def name
    return "Add/Replace Residential SIP Walls"
  end

  # human readable description
  def description
    return "This measure creates SIP constructions for the exterior walls adjacent to the living space."
  end
  ...
  def run(model, runner, user_arguments)
	...
	extinsfinwall.insertLayer(layercount,spline)
	layercount += 1
	extinsfinwall.insertLayer(layercount,wallins)
	layercount += 1
	extinsfinwall.insertLayer(layercount,spline)
	layercount += 1
	extinsfinwall.insertLayer(layercount,intsheathing)
	layercount += 1
	...
	extinsunfinwall.insertLayer(layercount,spline)
	layercount += 1
	extinsunfinwall.insertLayer(layercount,wallins)
	layercount += 1
	extinsunfinwall.insertLayer(layercount,spline)
	layercount += 1
	extinsunfinwall.insertLayer(layercount,intsheathing)
