...
class ResidentialCookingRangeGasTest < MiniTest::Test
  ...
  def test_multifamily_new_construction
    ...
    args_hash["c_ef"] = 0.4
    args_hash["o_ef"] = 0.058
    _test_measure(osm_geo_multifamily_3_units_beds, args_hash, 0, 2*num_units, 81, 225, num_units)
  end
  ...
  def test_multifamily_retrofit_replace
    ...
    args_hash["c_ef"] = 0.4
    args_hash["o_ef"] = 0.058
    model = _test_measure(osm_geo_multifamily_3_units_beds, args_hash, 0, 2*num_units, 81, 225, num_units)
    args_hash = {}
    args_hash["c_ef"] = 0.2
    args_hash["o_ef"] = 0.02
    _test_measure(model, args_hash, 2*num_units, 2*num_units, 201, 225, 2*num_units)
  end
  ...
  def test_argument_error_c_ef_lt_0
    ...
    assert_equal(result.errors[0].logMessage, "Cooktop energy factor must be greater than 0 and less than or equal to 1.")
  end
  ...
  def test_argument_error_o_ef_lt_0
    ...
    args_hash["o_ef"] = -1.0
    result = _test_error(osm_geo_beds, args_hash)
    assert_equal(result.errors[0].logMessage, "Oven energy factor must be greater than 0 and less than or equal to 1.")
  end
