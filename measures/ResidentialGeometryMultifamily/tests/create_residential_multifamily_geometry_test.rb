...
class CreateResidentialMultifamilyGeometryTest < MiniTest::Test
  ...
  def test_warning_uneven_units_per_floor_with_interior_corr
    ...
    result = _test_error(nil, args_hash)
    assert(result_warnings(result).size == 1)
    assert_equal("Success", result_value(result))
    assert_equal(result_warnings(result)[0], "Specified a double-loaded corridor and an odd number of units per floor. Subtracting one unit per floor.")
  end
  ...
  def test_error_no_corr
    ...
    assert(result_errors(result).size == 1)
    assert_equal("Fail", result_value(result))
    assert_equal(result_errors(result)[0], "Invalid corridor width entered.")
  end
  ...
  def test_warning_balc_but_no_inset
    ...
    assert(result_warnings(result).size == 1)
    assert_equal("Success", result_value(result))
    assert_equal(result_warnings(result)[0], "Specified a balcony, but there is no inset.")
  end
