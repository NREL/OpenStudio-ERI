...
class CreateResidentialMultifamilyGeometryTest < MiniTest::Test
  ...
  def test_warning_uneven_units_per_floor_with_interior_corr
    ...
    assert_equal(result.warnings.map{ |x| x.logMessage }[0], "Specified a double-loaded corridor and an odd number of units per floor. Subtracting one unit per floor.")
  end
  ...
  def test_error_no_corr
    ...
    assert(result.errors.size == 1)
    assert_equal("Fail", result.value.valueName)
    assert_equal(result.errors.map{ |x| x.logMessage }[0], "Invalid corridor width entered.")
  end
  ...
  def test_warning_balc_but_no_inset
    ...
    assert(result.warnings.size == 1)
    assert_equal("Success", result.value.valueName)
    assert_equal(result.warnings.map{ |x| x.logMessage }[0], "Specified a balcony, but there is no inset.")
  end
