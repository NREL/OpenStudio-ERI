...
class OSMeasures    
  ...
  def self.build_measures_from_hpxml(building, weather_file_path)
    ...
    get_ceiling_constructions(building, measures)
  ...
  def self.get_beds_and_baths(building, measures)
    ...
    args = {
            ...
            "num_bathrooms"=>"2" # Arbitrary, not used by RESNET
           }  
    measures[measure_subdir] = args
  ...
  def self.get_ceiling_constructions(building, measures)
  ...
  def self.get_heating_system(building, measures)
    ...
    if element_exists(htgsys.elements["HeatingSystemType/Furnace"])
      ...
      afue = htgsys.elements["AnnualHeatingEfficiency[Units='AFUE']/Value"].text.to_f
      ...
      percent = htgsys.elements["AnnualHeatingEfficiency[Units='Percent']/Value"].text.to_f
  ...
  def self.get_cooling_system(building, measures)
    ...
    if clgsys.elements["CoolingSystemType"].text == "central air conditioning"
      ...
      seer = clgsys.elements["AnnualCoolingEfficiency[Units='SEER']/Value"].text.to_f
      seer_adj = clgsys.elements["extension/PerformanceAdjustmentSEER"].text.to_f
    
      if seer < 16
        measure_subdir = "ResidentialHVACCentralAirConditionerSingleSpeed"
        args = {
                "seer"=>(seer*seer_adj).to_s,
                "eer"=>"11.1",
                "shr"=>"0.73",
      ...
      eer = htgsys.elements["AnnualCoolingEfficiency[Units='EER']/Value"].text.to_f
  ...
  def self.get_heat_pump(building, measures)
    ...
    if hp.elements["HeatPumpType"].text == "air-to-air"        
      ...
      seer = hp.elements["AnnualCoolingEfficiency[Units='SEER']/Value"].text.to_f
      seer_adj = hp.elements["extension/PerformanceAdjustmentSEER"].text.to_f
      hspf = htgsys.elements["AnnualHeatingEfficiency[Units='HSPF']/Value"].text.to_f
      hspf_adj = htgsys.elements["xtension/PerformanceAdjustmentHSPF"].text.to_f
      
      if seer < 16
        measure_subdir = "ResidentialHVACAirSourceHeatPumpSingleSpeed"
        args = {
                "seer"=>(seer*seer_adj).to_s,
                "hspf"=>(hspf*hspf_adj).to_s,
                "eer"=>"11.4",
                "cop"=>"3.05",
                "shr"=>"0.73",
                "fan_power_rated"=>"0.365",
      ...
      eer = hp.elements["AnnualCoolingEfficiency[Units='EER']/Value"].text.to_f
      cop = htgsys.elements["AnnualHeatingEfficiency[Units='COP']/Value"].text.to_f
  ...
  def self.get_refrigerator(building, measures)
    ...
    ref = building.elements["BuildingDetails/Appliances/Refrigerator"]
  ...
  def self.get_cooking_range(building, measures)
    ...
    if crange.elements["FuelType"].text == "electricity"
      ...
      args = {
              "fuel_type"=>{"natural gas"=>Constants.FuelTypeGas, "fuel oil"=>Constants.FuelTypeOil, "propane"=>Constants.FuelTypePropane}[crange.elements["FuelType"].text],
              "c_ef"=>"0.4",
              "o_ef"=>"0.058",
              "e_ignition"=>true,
              "mult"=>"1",
