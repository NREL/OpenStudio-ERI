...
# start the measure
class ProcessBoilerPropane < OpenStudio::Ruleset::ModelUserScript

  # human readable name
  def name
    return "Set Residential Boiler Propane"
  end
  ...
  def run(model, runner, user_arguments)
    ...
    boiler_aux = 220.0 # W during operation
    ...
    boiler.setFuelType(HelperMethods.eplus_fuel_map(Constants.FuelTypePropane))
    if boilerOutputCapacity != Constants.SizingAuto
...
ProcessBoilerPropane.new.registerWithApplication