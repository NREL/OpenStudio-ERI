...
class AirflowTest < MiniTest::Test
  ...
  def test_non_ducted_hvac_equipment
    ...
    assert_equal("Success", result.value.valueName)
    assert_includes(result.warnings.map{ |x| x.logMessage }[0], "No ducted HVAC equipment was found but ducts were specified. Overriding duct specification.")
  end  
  ...
  def test_duct_location_basement_but_no_basement
    ...
    assert_equal(result.errors.map{ |x| x.logMessage }[0], "Duct location is basement, but the building does not have a basement.")
  end   
  ...
  def test_duct_norm_leak_to_outside
    ...
    assert(result.errors.size == 1)
    assert_equal("Fail", result.value.valueName)
    assert_equal(result.errors.map{ |x| x.logMessage }[0], "Duct leakage to outside was specified by we don't calculate fan air flow rate.")
  end
