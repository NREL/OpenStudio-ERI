...
# start the measure
class ProcessBoilerGas < OpenStudio::Ruleset::ModelUserScript

  # human readable name
  def name
    return "Set Residential Boiler Gas"
  end
  ...
  def run(model, runner, user_arguments)
    ...
    boiler_aux = 76.0 # W during operation
    ...
    boiler.setFuelType(HelperMethods.eplus_fuel_map(Constants.FuelTypeGas))
    if boilerOutputCapacity != Constants.SizingAuto
...
ProcessBoilerGas.new.registerWithApplication