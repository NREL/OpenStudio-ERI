...
class ResidentialHotWaterHeaterTanklessPropaneTest < MiniTest::Test
  ...
  def _test_measure(osm_file_or_model, args_hash, expected_num_del_objects, expected_num_new_objects, expected_values, num_infos=0, num_warnings=0)
    ...
    assert_in_epsilon(num_new_whs.to_f*Waterheater.calc_actual_tankvol(nil, Constants.FuelTypePropane, Constants.WaterHeaterTypeTankless), actual_values["TankVolume"], 0.01)
