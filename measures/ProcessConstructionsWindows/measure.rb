...
class ProcessConstructionsWindows < OpenStudio::Ruleset::ModelUserScript
  ...
  def arguments(model)
    ...
    #make an argument for material and labor cost
    material_cost_ip = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("material_cost_ip",true)
    material_cost_ip.setDisplayName("Material and labor costs for construction per area used ($/ft^2).")
  ...
  def run(model, runner, user_arguments)
    ...
    if userdefined_ufactor.nil?
      ufactor = OpenStudio::convert(selected_ufactor.get.to_SimpleGlazing.get.getUFactor.value,"W/m*K","Btu/hr*ft*R").get
      shgc = selected_shgc.get.to_SimpleGlazing.get.getSolarHeatGainCoefficient.value
    else
      ufactor = userdefined_ufactor
    ...
    sumDesSch.setName("WindowShadingScheduleSummer")
    winDesSch = ish_rule_days[0]
    winDesSch.setName("WindowShadingScheduleWinter")
    ...
    sumDesSch = ish_rule_days[6]
    sumDesSch.setName("WindowShadesSummer")
    winDesSch = ish_rule_days[0]
    ...
    # if not material_labor_cost.abs == 0
    ...
    #   lcc_mat = OpenStudio::Model::LifeCycleCost.createLifeCycleCost("LCC_Mat-#{}")
    # end
