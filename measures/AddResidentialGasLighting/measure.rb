...
class ResidentialGasLighting < OpenStudio::Ruleset::ModelUserScript
  ...
  def name
    return "Add/Replace Residential Gas Lighting"
  end
  ...
  def modeler_description
    return "Since there is no Gas Lighting object in OpenStudio/EnergyPlus, we look for a GasEquipment object with the name that denotes it is residential gas lighting. If one is found, it is replaced with the specified properties. Otherwise, a new such object is added to the model."
  end
  ...
  def arguments(model)
    ...
    living_space_type.setDescription("Select the living space type. The lighting will be located outdoors, but the living space floor area is needed to scale energy use.")
    ...
    fbasement_space_type.setDescription("Select the finished basement space type. The lighting will be located outdoors, but the finished basement space floor area is needed to scale energy use.")
  ...
  def run(model, runner, user_arguments)
    ...
    if scale_energy
        ...
        gl_ann_g = constant + nbr_coef * nbeds + cfa_coef * cfa_total # therm/yr
    else
	...
	#add lighting to the living space
    ...
    if has_gl == 0 
        ...
        gl.setName(obj_name)
        gl.setSpaceType(living_space_type)
    ...
    if replace_gl == 1
        runner.registerFinalCondition("The existing gas lighting has been replaced by one with #{gl_ann_g.round} therms annual energy consumption.")
    else
        runner.registerFinalCondition("Gas lighting has been added with #{gl_ann_g.round} therms annual energy consumption.")
    end
