...
class ResidentialHotWaterHeaterTanklessElectricTest < MiniTest::Test
  ...
  def test_new_construction_standard_living
    ...
    _test_measure(osm_geo_beds_loc, args_hash, 0, 1, Constants.LivingZone, 100000000.0, 0.911, 125, 0, 0, 1, 0)
  end

  def test_new_construction_standard_setpoint_130
    args_hash = {}
    args_hash["dhw_setpoint_temperature"] = 130
    _test_measure(osm_geo_beds_loc, args_hash, 0, 1, Constants.FinishedBasementZone, 100000000.0, 0.911, 130, 0, 0, 1, 0)
  end

  def test_new_construction_standard_cd_0
    args_hash = {}
    args_hash["water_heater_cycling_derate"] = 0
    _test_measure(osm_geo_beds_loc, args_hash, 0, 1, Constants.FinishedBasementZone, 100000000.0, 0.99, 125, 0, 0, 1, 0)
  end

  def test_retrofit_replace
    args_hash = {}
    model = _test_measure(osm_geo_beds_loc, args_hash, 0, 1, Constants.FinishedBasementZone, 100000000.0, 0.911, 125, 0, 0, 1, 0)
    args_hash = {}
    args_hash["rated_energy_factor"] = 0.999
    _test_measure(model, args_hash, 1, 1, Constants.FinishedBasementZone, 100000000.0, 0.919, 125, 0, 0, 1, 0)
  end
  
  def test_retrofit_replace_tank_gas
    args_hash = {}
    _test_measure(osm_geo_beds_loc_tank_gas, args_hash, 1, 1, Constants.FinishedBasementZone, 100000000.0, 0.911, 125, 0, 0, 1, 0)
  end

  def test_retrofit_replace_tank_oil
    args_hash = {}
    _test_measure(osm_geo_beds_loc_tank_oil, args_hash, 1, 1, Constants.FinishedBasementZone, 100000000.0, 0.911, 125, 0, 0, 1, 0)
  end

  def test_retrofit_replace_tank_propane
    args_hash = {}
    _test_measure(osm_geo_beds_loc_tank_propane, args_hash, 1, 1, Constants.FinishedBasementZone, 100000000.0, 0.911, 125, 0, 0, 1, 0)
  end

  def test_retrofit_replace_tank_electric
    args_hash = {}
    _test_measure(osm_geo_beds_loc_tank_electric, args_hash, 1, 1, Constants.FinishedBasementZone, 100000000.0, 0.911, 125, 0, 0, 1, 0)
  end

  def test_retrofit_replace_tankless_gas
    args_hash = {}
    _test_measure(osm_geo_beds_loc_tankless_gas, args_hash, 1, 1, Constants.FinishedBasementZone, 100000000.0, 0.911, 125, 0, 0, 1, 0)
  end

  def test_retrofit_replace_tankless_propane
    args_hash = {}
    _test_measure(osm_geo_beds_loc_tankless_propane, args_hash, 1, 1, Constants.FinishedBasementZone, 100000000.0, 0.911, 125, 0, 0, 1, 0)
  end

  def test_multifamily_new_construction
    num_units = 3
    args_hash = {}
    _test_measure(osm_geo_multifamily_3_units_beds_loc, args_hash, 0, num_units, nil, 300000000.0, 2.733, 375, 0, 0, num_units, 0)
  end
  ...
  def test_multifamily_retrofit_replace
    num_units = 3
    args_hash = {}
    model = _test_measure(osm_geo_multifamily_3_units_beds_loc, args_hash, 0, num_units, nil, 300000000.0, 2.733, 375, 0, 0, num_units, 0)
    args_hash = {}
    args_hash["rated_energy_factor"] = 0.999
    _test_measure(model, args_hash, num_units, num_units, nil, 300000000.0, 2.757, 375, 0, 0, num_units, 0)
  end
  ...
  def test_argument_error_capacity_lt_0
    ...
    result = _test_error(osm_geo_beds_loc, args_hash)
    assert_equal(result_errors(result)[0], "Nominal capacity must be greater than 0.")
  end
  ...
  def test_argument_error_ef_lt_0
    ...
    result = _test_error(osm_geo_beds_loc, args_hash)
    assert_equal(result_errors(result)[0], "Rated energy factor must be greater than 0 and less than 1.")
  end
  ...
  def test_argument_error_cd_lt_0
    ...
    result = _test_error(osm_geo_beds_loc, args_hash)
    assert_equal(result_errors(result)[0], "Cycling derate must be at least 0 and at most 1.")
  end
