...
class AddOSWaterHeaterMixedStorageOil < OpenStudio::Ruleset::ModelUserScript
  ...
  def name
    return "AddOSWaterHeaterMixedStorageOil"
  end
  ...
  def arguments(model)
    ...
    bedrooms_choices = (1..5).map { |i| i.to_s }
    number_of_bedrooms = osargument::makeChoiceArgument("number_of_bedrooms",bedrooms_choices, true)
    ...
    storage_tank_volume.setDisplayName("Volume of the Storage Tank (gallons) of the Oil Hot Water Heater. Set to 0 to have Storage tank volume autosized.")
    ...
    rated_energy_factor.setDisplayName("Rated Energy Factor of Oil Storage Tank Water Heater.")
    ...
    water_heater_location.setDisplayName("Thermal Zone where the Oil Storage Water Heater is located")
    ...
    water_heater_capacity.setDisplayName("The nominal capacity [kBtu/hr] of the gas storage water heater. Set to 0 to have this field autosized.")
    ...
    water_heater_recovery_efficiency.setDisplayName("Rated Recovery Efficiency of the water heater equal to the ratio of energy delivered to the water to the energy content of the fuel consumed by the water heater. Test procedures to test recovery efficiency are defined by the DOE in 10 CFR Part 430, Appendix E to Subpart B. Enter his number as a percent. This is used to calculate the thermal efficiency. Users wil need to review the E+out.eio file to confirm Calculated recovery efficiency - Rated recovery efficiency.")
  ...
  def create_new_heater
    ...
    new_heater.setHeaterFuelType("FuelOil#2")
  ...
  def configure_cycle_loss_coeficients(new_heater)
    ...
    if (@args[:water_heater_capacity].value == 0) # flag for autosized capacity
      # use midpoint of valid capacity range (90 - 105 kBtu/hr)
      rated_input_power = qty(97.5, KBtuhr)
    else      
  ...
  def register_info_messages(new_heater)
    ...
    on_cycle_loss_coeff = new_heater.onCycleLossCoefficienttoAmbientTemperature
    off_cycle_loss_coeff = new_heater.offCycleLossCoefficienttoAmbientTemperature
  ...
  def parse_arguments
    return {
      number_of_bedrooms: @runner.getStringArgumentValue("number_of_bedrooms", @user_arguments),
  ...
  def validate_storage_tank_volume
    ...
    if storage_tank_volume < 30
      @runner.registerWarning("A storage tank volume of less than 30 gallons and a certified rating is not commercially available. Please review the input.")
    end                             
    if storage_tank_volume > 50
      @runner.registerWarning("A hot water heater with a storage tank volume of greater than 50 gallons and a certified rating is not commercially available. Please review the input.")
    end                             
  end
  ...
  def validate_rated_energy_factor
    ...
    if (rated_energy_factor >0.68)
      @runner.registerWarning("Rated Energy Factor for Commercially available Oil Storage Water Heaters should be less than 0.68")
    end    
    if (rated_energy_factor <0.51)
      @runner.registerWarning("Rated Energy Factor for Commercially available Oil Storage Water Heaters should be greater than 0.51")
    end    
  end
  ...
  def validate_water_heater_effectiveness
    ...
    if (water_heater_effectiveness < 0)
      @runner.registerError("Oil Water Heater Heat Exchange Effectiveness must be greater than 0")
    end
    if (water_heater_effectiveness > 1)
      @runner.registerError("Oil Water Heater Heat Exchange Effectiveness must be less than 1")
    end
  ...
  def validate_water_heater_capacity
    ...
    if water_heater_capacity < 0
      @runner.registerError("Oil Storage Water Heater Nominal Capacity must be greater than 0 kBtu/hr.")
    end

    if water_heater_capacity < 90
      @runner.registerWarning("Commercially Available Oil Storage Water Heaters should have a minimum Nominal Capacity of 90 kBtu/h.")
    end
    if water_heater_capacity > 105
      @runner.registerWarning("Commercially Available Oil Storage Water Heaters should have a maximum Nominal Capacity of 105 kBtu/h.")
    end
  end
  ...
  def validate_water_heater_recovery_efficiency
    ...
    if (water_heater_recovery_efficiency < 0)
      @runner.registerError("Oil Storage Water Heater Recovery Efficiency must be between 0.0 and 1.0.")
    end
    if (water_heater_recovery_efficiency > 1)
      @runner.registerError("Oil Storage Water Heater Recovery Efficiency must be between 0.0 and 1.0.")
    end
    if (water_heater_recovery_efficiency <= energy_factor)
      @runner.registerError("Recovery Efficiency must be greater than Rated Energy Factor.")
    end
    
    if (water_heater_recovery_efficiency < 0.75)
    ...
    if (water_heater_recovery_efficiency > 0.90)
      @runner.registerWarning("Commercially Available Oil Storage Water heaters should have a maximum rated recovery efficiency of 0.90.")
    end
  end
